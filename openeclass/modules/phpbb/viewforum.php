...
$forum = intval(htmlspecialchars($_GET['forum']));
...
if (isset($_GET['start'])) {
        $first_topic = intval(htmlspecialchars($_GET['start']));
} else {
	$first_topic = 0;
}
...
if (mysql_num_rows($result) > 0) { // topics found
	while($myrow = mysql_fetch_array($result)) {
		...
		if (isset($_GET['start']) and $_GET['start'] > 0) {
			...
			$tool_content .= "<a href='".htmlspecialchars($_SERVER['PHP_SELF'])."?forum=$forum&start=$s&amp;topicnotify=$topic_link_notify&amp;topic_id=$myrow[topic_id]'>
